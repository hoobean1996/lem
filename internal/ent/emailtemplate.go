...
func (e EmailTemplateEdges) AppOrErr() (*App, error) {
	if e.App != nil {
		return e.App, nil
...
func (*EmailTemplate) scanValues(columns []string) ([]any, error) {
	...
	for i := range columns {
		...
		case emailtemplate.FieldName, emailtemplate.FieldDescription, emailtemplate.FieldSubject, emailtemplate.FieldBodyHTML, emailtemplate.FieldBodyText:
			values[i] = new(sql.NullString)
		case emailtemplate.FieldCreatedAt, emailtemplate.FieldUpdatedAt:
			values[i] = new(sql.NullTime)
		case emailtemplate.ForeignKeys[0]: // app_email_templates
			values[i] = new(sql.NullInt64)
		default:
...
func (_m *EmailTemplate) assignValues(columns []string, values []any) error {
	...
	for i := range columns {
		...
		case emailtemplate.ForeignKeys[0]:
			if value, ok := values[i].(*sql.NullInt64); !ok {
				return fmt.Errorf("unexpected type %T for edge-field app_email_templates", value)
			} else if value.Valid {
				_m.app_email_templates = new(int)
				*_m.app_email_templates = int(value.Int64)
			}
		default:
...
// QueryApp queries the "app" edge of the EmailTemplate entity.
func (_m *EmailTemplate) QueryApp() *AppQuery {
	return NewEmailTemplateClient(_m.config).QueryApp(_m)
}

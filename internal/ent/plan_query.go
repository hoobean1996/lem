...
func (_q *PlanQuery) QueryApp() *AppQuery {
...
func (_q *PlanQuery) WithApp(opts ...func(*AppQuery)) *PlanQuery {
	query := (&AppClient{config: _q.config}).Query()
...
func (_q *PlanQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Plan, error) {
	...
	if query := _q.withApp; query != nil {
		if err := _q.loadApp(ctx, query, nodes, nil,
			func(n *Plan, e *App) { n.Edges.App = e }); err != nil {
			return nil, err
		}
	}
...
func (_q *PlanQuery) loadApp(ctx context.Context, query *AppQuery, nodes []*Plan, init func(*Plan), assign func(*Plan, *App)) error {
	ids := make([]int, 0, len(nodes))
	nodeids := make(map[int][]*Plan)
	for i := range nodes {
		if nodes[i].app_plans == nil {
			continue
		}
		fk := *nodes[i].app_plans
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "app_plans" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}

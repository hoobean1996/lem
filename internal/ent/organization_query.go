...
func (_q *OrganizationQuery) QueryApp() *AppQuery {
...
func (_q *OrganizationQuery) QuerySubscriptions() *SubscriptionQuery {
...
func (_q *OrganizationQuery) WithApp(opts ...func(*AppQuery)) *OrganizationQuery {
	query := (&AppClient{config: _q.config}).Query()
...
func (_q *OrganizationQuery) WithSubscriptions(opts ...func(*SubscriptionQuery)) *OrganizationQuery {
	query := (&SubscriptionClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *OrganizationQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Organization, error) {
	...
	if query := _q.withApp; query != nil {
		if err := _q.loadApp(ctx, query, nodes, nil,
			func(n *Organization, e *App) { n.Edges.App = e }); err != nil {
			return nil, err
		}
	}
	...
	if query := _q.withSubscriptions; query != nil {
		if err := _q.loadSubscriptions(ctx, query, nodes,
			func(n *Organization) { n.Edges.Subscriptions = []*Subscription{} },
			func(n *Organization, e *Subscription) { n.Edges.Subscriptions = append(n.Edges.Subscriptions, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *OrganizationQuery) loadApp(ctx context.Context, query *AppQuery, nodes []*Organization, init func(*Organization), assign func(*Organization, *App)) error {
	ids := make([]int, 0, len(nodes))
	nodeids := make(map[int][]*Organization)
	for i := range nodes {
		if nodes[i].app_organizations == nil {
			continue
		}
		fk := *nodes[i].app_organizations
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "app_organizations" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (_q *OrganizationQuery) loadMembers(ctx context.Context, query *OrganizationMemberQuery, nodes []*Organization, init func(*Organization), assign func(*Organization, *OrganizationMember)) error {
	...
	for _, n := range neighbors {
		fk := n.organization_members
		if fk == nil {
			return fmt.Errorf(`foreign-key "organization_members" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "organization_members" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
...
func (_q *OrganizationQuery) loadInvitations(ctx context.Context, query *OrganizationInvitationQuery, nodes []*Organization, init func(*Organization), assign func(*Organization, *OrganizationInvitation)) error {
	...
	for _, n := range neighbors {
		fk := n.organization_invitations
		if fk == nil {
			return fmt.Errorf(`foreign-key "organization_invitations" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
...
func (_q *OrganizationQuery) loadSubscriptions(ctx context.Context, query *SubscriptionQuery, nodes []*Organization, init func(*Organization), assign func(*Organization, *Subscription)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int]*Organization)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	query.Where(predicate.Subscription(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(organization.SubscriptionsColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	...
	for _, n := range neighbors {
		fk := n.organization_subscriptions
		if fk == nil {
			return fmt.Errorf(`foreign-key "organization_subscriptions" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "organization_subscriptions" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil

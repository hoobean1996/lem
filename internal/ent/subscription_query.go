...
func (_q *SubscriptionQuery) QueryApp() *AppQuery {
...
func (_q *SubscriptionQuery) WithApp(opts ...func(*AppQuery)) *SubscriptionQuery {
	query := (&AppClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
//		GroupBy(subscription.FieldStatus).
//		Aggregate(ent.Count()).
//		Scan(ctx, &v)
...
//
//	client.Subscription.Query().
//		Select(subscription.FieldStatus).
//		Scan(ctx, &v)
...
func (_q *SubscriptionQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Subscription, error) {
	...
	if _q.withUser != nil || _q.withOrganization != nil || _q.withApp != nil || _q.withPlan != nil {
		withFKs = true
	}
	if withFKs {
	...
	if query := _q.withApp; query != nil {
		if err := _q.loadApp(ctx, query, nodes, nil,
			func(n *Subscription, e *App) { n.Edges.App = e }); err != nil {
			return nil, err
		}
	}
...
func (_q *SubscriptionQuery) loadOrganization(ctx context.Context, query *OrganizationQuery, nodes []*Subscription, init func(*Subscription), assign func(*Subscription, *Organization)) error {
	...
	nodeids := make(map[int][]*Subscription)
	for i := range nodes {
		if nodes[i].organization_subscriptions == nil {
			continue
		}
		fk := *nodes[i].organization_subscriptions
		if _, ok := nodeids[fk]; !ok {
			ids = append(ids, fk)
		}
		nodeids[fk] = append(nodeids[fk], nodes[i])
	...
	for _, n := range neighbors {
		nodes, ok := nodeids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "organization_subscriptions" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (_q *SubscriptionQuery) loadApp(ctx context.Context, query *AppQuery, nodes []*Subscription, init func(*Subscription), assign func(*Subscription, *App)) error {
	ids := make([]int, 0, len(nodes))
	nodeids := make(map[int][]*Subscription)
	for i := range nodes {
		if nodes[i].app_subscriptions == nil {
			continue
		}
		fk := *nodes[i].app_subscriptions
		if _, ok := nodeids[fk]; !ok {
			ids = append(ids, fk)
		}
		nodeids[fk] = append(nodeids[fk], nodes[i])
	...
	for _, n := range neighbors {
		nodes, ok := nodeids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "app_subscriptions" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
